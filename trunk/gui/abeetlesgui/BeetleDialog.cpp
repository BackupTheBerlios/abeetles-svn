...
BeetleDialog::BeetleDialog( CBeetle * beetle,QWidget * parent, Qt::WindowFlags f):QDialog(parent,f)
{
	...
	HungryThresholdLabel=new QLabel("Hungry Threshold: "+QString::number(beetle->HungryThreshold));
	IdLabel=new QLabel("Id: "+QString::number(beetle->Id));
	InvInChildLabel=new QLabel("Investment In Child: "+QString::number(beetle->InvInChild));
	LearnAbilityLabel=new QLabel("Learning Ability: "+QString::number(beetle->LearnAbility));
	NumChildrenLabel=new QLabel("Number of Children: "+QString::number(beetle->NumChildren));
	...
	QLabel * brainLabel=new QLabel("Brain (N-nothing, W-wall, B-beetle,F-flower, S-step, L-rotate left, R-rotate right, in colums is what is on the right and in rows is what is on the left):");
