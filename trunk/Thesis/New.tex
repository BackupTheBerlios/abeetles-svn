...
Other possibilities were Win32API and MFC. The first is not class based and difficult to use. MFC is a class based library, it is distributed freely with MS  Visual Studio, but the level of abstraction is lower then in Qt.
...
Functionality of Abeetles is divided into following components: Environment, Run of Script, Run of Gui, Statistics and Configuration Manager. Configuration Manager encapsulates functions for saving and loading of an environment. In figure ... these components are represented by their key classes.
...
The idea of graphical user interface of Abeetles was inspired by interface of Broucci, see Figure \ref{img.Broucci} on page \pageref{img.Broucci}. In Abeetles agents are also represented by arrows showing their direction. %figure

Layout of window of GUI of Abeetles has much in common with qt-view of Abeetles,  see Figure \ref{img.Avida} on page \pageref{img.Avida} . The reason is the similarity of grid base of the environment and intention to serve as a scientific tool. 
