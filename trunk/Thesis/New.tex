...
\section {Model of Abeetles}
Artificial life in simulator Abeetles simulates natural life as environment where live agents. Agents element of environment that are subject to evolution and environment is all the world with other not evolved objects where agents live.

Agents in Abeetles are called beetles and generally they all are of the same species as they can mate with each other and share the same static part of their behavior. Environment is a grid in form of toroid. In grid are placed three types of objects. Beetles are the only inhabitants of the environment that are able to move. Another type of objects are flowers. Flowers grow in cells of the environment and serve as food for beetles. In environment are also placed walls. Walls are static obstacles in cells that cause that no beetle can access that cell and therefore serve for forming of the shape of the environment.

Beetles will contain static part, that can not be evolved, evolvable part and part that can be influenced during life by learning. Basic features will be level of energy, decision mechanism that takes as input what the beetle see and in which state he is and outputs a decision what to do and expectation on a partner for mating. 

The simulator runs the artificial model described above. Objectives of the simulator imply requirements on the application. Which features it should have and which functionality it should offer to users.

As the simulator should serve for 
