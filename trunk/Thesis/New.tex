...
The current proggres in Artificial life is regulary presented at Alife -International Conference on the Simulation and Synthesis of Living Systems and ECAL - European Conference on Artificial Life organized by International Society of Artificial Life (ISAL), \url{http://www.alife.org}. 
...
This thesis is bounded within software approach to Artificial life. Its main target is to design and create software simulator Abeetles. Abeetles runs life of simple organisms in its specific environment and is concerned with evolution of their features and behavior. The system avails experiments with evolution of the organisms under various conditions and offer overviews and statistics of the proccess. Experiments performed with the system will be described in chapter Experiments.
...
The purpose of Abeetles is to be a scientific simulator. But not only for users from comunity of computer scientists, but also for those who besides fast results also appreciate user friendly interface with visualisation, even if it decreases performance of the system. 
...
six cathegories: Agents, Mating, Environment, User interface settings and Implementation. Symbol (?) in the classification means that features of the simulator from this aspect are unknown.
...
\subsection {Bitozoa}
Bitozoa is a simulator created by M. Borkowski, that uses for representation of agents neural network in combination with a set of parameters. It evolves two types of agents, herbivores and carnivores. It was created for purpose of amusement and education. Used version is Bitozoa 2 - artificial life and artficial intelligence simulation, 2000. Web page of the project is \url{http://www.bpp.com.pl/bitozoa2/bitozoa2.html}.
...
Simulator Primordial life offers possiblity to connect simulators and in order to it connect environments together. Plan of Abeetles originally included computation distributed on more computers, but finally it is only single compute r application. Reasons are described later. 

Obstacles are incomporated in none of the five simulators. But obstacles and consequenly the shape of the environment can be very interesting for Abeetles, which evolves strategy of motion.  
...
Artificial life in simulator Abeetles is an environment inhabited by agents. Agents are those elements of environment that are subject to evolution and environment is all the world with other not evolved objects where agents live. The conceptual model key objects in Abeetles is shown in Figure \ref{obr.AnalysisDataModel}.

%Beetles - agents with static part and genome. Flowers. Walls
Agents in Abeetles are called beetles and generally they all are instances of the same model, as they share the same static part as well as constrains on the variable part of their behavior. It can be also influenced by learning.  The variable part makes up the genome and it is altered by evolution. Relationships among beetles are created by mating and bearing of children. 

The environment is a grid in the form of a toroid. In the grid three types of objects are placed. Beetles are the only inhabitants of the environment that are able to move. Another type of objects are flowers. Flowers grow in cells of the environment and serve as food for beetles. Neither beetles nor flowers can occur in cells of the grid, where are walls. Walls are static obstacles that serve for forming of the shape of the environment.
...
UML sequence diagram is snown in Figure. The expectations on the system in this case are:
\begin{itemize}
...
In Abeetles the environment is a grid in the form of a toroid. Beetles see only three neihbouring cells. As described later, growth of flowers is dependent only on a setting of each cell. Therefore interactions are only local and paralelization could be convenient. 
...
Platform for Abeetles is MS Windows XP. %some reasons??
...
Other possibilities were Win32API and MFC. The first is not class based and difficult to use. MFC is a class based library, it is distributed freely with MS  Visual Studio, but the level of abstraction is lower then in Qt and future support of this library by Microsoft is not sure.


\section{Data model of Abeetles}
%Beetle and its attributes, environment and its attributes
The basic model of life in Abeetles is described in chapter Analysis. Now decomposition proceedes and data structures are designed.
The key data structures of Abeetles are beetles and an environment.
A beetle has following evolvable features:
\begin {itemize}
\item Brain - decision function (situation) -> action. The situation is combination of what the beetle sees and his inner states. States of a Beetle are whether he is hungyry or not. It sees what is on the left,in the front and on the right.
\item Expectation on parter - A beetle check before mating, whether the partner is fullfils its expectations on amount of energy, investment in children, learning ability and age.
\item Hungry threshold - Threshold defines whether the beetle is or is not hungry.  
\item Investment in children - The beetle invests this amount of energy into its new born offspring.
\item Learning Ability - The beetle expects certain level of learning ability from its partner.
\end {itemize}
...
Functionality of Abeetles is divided into following components: Environment, Run of Script, Run of Gui, Statistics and Configuration Manager. Configuration Manager encapsulates functions for saving and loading of an environment. In \ref {img.ComponentModel} these components are represented by their key classes.

\section {User interface}
\subsection {Graphical user interface}
The idea of graphical user interface of Abeetles was inspired by interface of Broucci, see Figure \ref{img.Broucci} on page \pageref{img.Broucci}, in visualization of beetles. In Abeetles agents are also represented by arrows showing their direction. %figure

Layout of window of GUI of Abeetles resembles the qt-view of Avida,  see Figure \ref{img.Avida} on page \pageref{img.Avida} . The reason is the similarity of grid base of the environment and intention to serve as a scientific tool.

 
\subsection {Statistics}

Every artifical life simulator that endeavors after more that being just a game includes some kind of statistics. 
...
\item Mitozoos: genetic landscape, log of events, on reproduction the crossjoin of genotypes is shown graphicaly, details about an agent.
...
It can be compared with result of run of Experiment 1 and its histogram after 30,000 updates in \ref{Exp1_1_histExp.eps}. It shows that random expectations on a partner allow opportunity for value to vary more, but tendences are not so different from result of case when the ranges of expectations were the widest possible. In that case it is only expectation on energy that changed significantly and can be in connection with the fact that energy at the time of mating is already restricted by hungry threshold. We expected the expectation on investment in child to grow to at least fourth of maximum of energy, but it developed to about 4 (but did not shrink under 2) in case of random initial values and in case of the widest ranges remained 0 for most of beetles but a tenth of them. Even it this case only one beetle in 144 invests less than 2. The result implies that it is enough for a new-born beetle to get 4 pieces of energy at the beginning when costs are 2,2,3,1.

\section{Experiment2 - Species of beetles}

This experiment have for its objective to explore possibilities of development of species of beetles. Species of beetles are defined for this purpose as such groups of beetles that members of one group can never mate with beetles from another group. The difference that would cause such an incompatibility can only sping up when expectations and respective features do not match. 
...
In figure \ref{img.Exp1_Sub2_1_histExp} and figure \ref{img.Exp1_1_histExp.eps} is visible that values of expectations on investment in children have very restricted range and from figure \ref{img.four_caves_run1_hist30000} and other descriptions of results of Experiment 1 is clear that values of investment in children behave similarly.

Development of learning abilities in Experiment 1 was more hopeful, as the values narrowed to two separated ranges, see \ref{img.four_caves_run1_hist30000}. When the situation of the environment is viewed, details of beetles proove that beetles with learning ability 34 live in the bottom left cave and beetles with learning ability 46 live in the bottom right cell (the other two cells are not inhabited). Thus beetles really developed to two different groups, which is for species very promising, even though expectations, see \ref{Exp1_1_histExp.eps}, are a wide range over almost all possible values. Therefore this result will be used for this experiment 
...
The first attempt was to increase \textbf{mutation rate} so as to get expectations to follow respective values. The result was that also the value of learning ability started to vary swiftly and also the beetles became extinct in several thousands updates.
...
The second attempt was to run the environment much longer and leave enough time for evolution to do its work. After roughly 300,000 updates two species of beetles separated, see figure \ref{img.Exp1_1_hist365000} and figure \ref{img.Exp1_1_hist365000Exp}.
