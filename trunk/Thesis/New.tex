...
The first question is whether computation of artificial life of Abeetles can be split into independent subtasks. Generally, the run of an artificial life simulator is a sequnce of more or less detached updates. In each update each  object in the environment gets opportunity to make a small number of actions, usually one, with one input of information from the environment, e.g. snapshot of its surroundings. 

One suitable possibility of division of the task is accordingly splitting of the environment into parts, that compute one update separately and after each update  interchange information about the edges that can be in view of object of other parts.

An alternative of this attitude ...

Environment is a grid in the form of a toroid. Interactions in the grid should be only local, so as to split the environment into several parts and each of them compute on a different machine. 
