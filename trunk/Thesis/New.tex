...
Systems compared with Abeetles show information either graphicaly or output them to a file (indicated in brackets):
\begin{itemize}
...
\item Primordial life: information about environment (population, time death and birth rate numbers), global status of all connected ecosystems, information about an agent.
\end {itemize}

%Possible to add pictures of statistics from other programms.

These simulators prefer to show statistics graphicaly or textualy in the program. But for further processing output to files in a standard format might be useful. Abeetles writes statistics into files, only several actual numbers shows in the graphical interface. Abeetles as well as all described simulators shows details of an individual. It counts actual information about environment like time, number of agents, some average values, etc. Graphs of development in time are offered by Bitozoa and Gene Pool. They are very useful for observation of what is going on in the simulator within time. Abeetles does not show graphs directly, but saves values into a file in comma separated value format. Graphs can be then drawn by a special program, e.g. MS Excel. In this format changes of number of beetles, number of flowers and number of births within time are stored. It is also used for graphs of frequency o distribution of values of age,  learning abilities, investment in children and energy.

  
\subsection {Script}

Script for Abeetles is intended to be easy to understand. It uses format
"command = value" and avails to input any number of runs of the simulator. For each of them all necessary parameters can be defined.
...
Class CBeetle represents a beetle with all respective features. The most interesting process connected with this class in mating of two beetles. First, demands of both beetles are checked by function IsExpOnPartnerSatisfied(). When beetles find themselves attracted in the other one, CreateChild() operation is called.  This method first applies Crossover1Point(),contains the genetic crossover algorithm, and generates two chromosomes. One of them is randomly chosen for the new-born descendant. The descendant then undergoes random changes in operation Mutation(). The new beetle is then placed in one of four cells that are neighboring for both parents. If none of them is empty, the beetle is immediately discarded. Afterwards it start to live its own life.

Another important process in the environment is the run of life in the environment. Instance of class CEnvironment is called $env$. Its attributes Grid and Grid\_Past are of type CGrid. Grid\_Past represents the grid in previous update. Using following code the new env.Grid, representing the grid of the new update is created.



\begin{verbatim}
for(I=0;I<env.Grid\_Past.G\_Width;I++)
  for(J=0;J<env.Grid\_Past.G\_Height;J++)	
  {
    if (env.Grid\_Past.GetCellContent(I,J)==BEETLE) 
      env.MakeBeetleAction(I,J);	
    else if (env.Grid\_Past.GetCellContent(I,J)==NOTHING) 
            env.MakeFlowerGrow(I,J);
         else if (env.Grid\_Past.GetCellContent(I,J)==FLOWER) 
                env.MakeFlowerDie(I,J);	
  }
\end{verbatim}			
  
			
Lorem ipsum dolor sit amet consectetuer mauris vitae urna malesuada condimentum. Accumsan sociis nec tristique ullamcorper facilisi.
...
%2nd - Components of Abeetles - Class diagram of CRunScript(+COneRun), MainWindow(+CBeetleDlg,CNewEnvDlg), CStatistEnv, CEnvironment, CfgManager  ajejich detailnejsi pomocne tridy
